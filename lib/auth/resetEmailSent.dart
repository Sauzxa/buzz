...
class ResetEmailSentPage extends StatefulWidget {
  ...
  const ResetEmailSentPage({super.key, required this.email});
...
class _ResetEmailSentPageState extends State<ResetEmailSentPage> {
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        Navigator.pushReplacement(
          context,
          FadeRoute(
            page: SetNewPasswordPage(email: widget.email, otp: _getOtpCode()),
          ),
        );
      } else {
  ...
  Future<void> _onResendCode() async {
    ...
    try {
      final authProvider = context.read<AuthProvider>();
      final success = await authProvider.resendResetOtp(widget.email);
